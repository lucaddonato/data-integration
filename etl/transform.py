...
def validate_nulls(df):
    ...
    if dropped:
        log.warning(f"Regra 1 (nulos): {dropped} registro(s) removido(s) por id/title nulo")
    else:
        log.info("Regra 1 (nulos): nenhum registro removido")
...
def validate_duplicates(df):
    ...
    dropped = before - len(df)
    if dropped:
        log.warning(f"Regra 2 (duplicatas): {dropped} registro(s) duplicado(s) removido(s)")
    else:
        log.info("Regra 2 (duplicatas): nenhuma duplicata encontrada")
    return df


def validate_ranges(df):
    """Regra 3 — Ranges: corrige valores fora dos limites aceitaveis."""
    # vote_average deve ser [0, 10]
    mask_va = ~df["vote_average"].between(0, 10)
    if mask_va.any():
        log.warning(f"Regra 3 (ranges): {mask_va.sum()} vote_average(s) fora de [0,10] → None")
        df = df.copy()
        df.loc[mask_va, "vote_average"] = None
    else:
        log.info("Regra 3 (ranges): todos os vote_average dentro de [0, 10]")

    # budget e revenue nao podem ser negativos
    mask_budget = df["budget"] < 0
    if mask_budget.any():
        log.warning(f"Regra 3 (ranges): {mask_budget.sum()} budget(s) negativo(s) → 0")
        df.loc[mask_budget, "budget"] = 0

    mask_revenue = df["revenue"] < 0
    if mask_revenue.any():
        log.warning(f"Regra 3 (ranges): {mask_revenue.sum()} revenue(s) negativo(s) → 0")
    ...
    if "runtime" in df.columns:
        ...
        if mask_runtime.any():
            log.warning(f"Regra 3 (ranges): {mask_runtime.sum()} runtime(s) invalido(s) → None")
...
def extract_genres(movies_raw):
    """Extrai todos os pares (genre_id, genre_name) unicos de todos os filmes."""
    genres = {}
...
def extract_languages(movies_raw):
    """Extrai todos os pares (language_code, language_name) unicos."""
    languages = {}
    for movie in movies_raw:
        ...
        if not code:
            continue
        name = code
        for lang in (movie.get("spoken_languages") or []):
...
def build_movie_genres(movies_raw, valid_ids):
    """Monta a lista de relacoes filme <-> genero para a tabela movie_genres."""
    relations = []
    for movie in movies_raw:
        mid = movie.get("id")
...
def transform():
    raw = load_raw()
    log.info(f"{len(raw)} filmes carregados do arquivo bruto")
    ...
    log.info(f"{len(df)} filmes apos todas as validacoes")
    ...
    log.info(f"movies_clean.json salvo com {len(movies_clean)} registros")

    # Salva dimensoes
    genres = extract_genres(raw)
    with open(GENRES_PATH, "w", encoding="utf-8") as f:
        json.dump(genres, f, ensure_ascii=False, indent=2)
    log.info(f"genres.json salvo com {len(genres)} generos")
    ...
    log.info(f"languages.json salvo com {len(languages)} idiomas")

    # Salva relacoes filme-genero
    movie_genres = build_movie_genres(raw, valid_ids)
    with open(RELATIONS_PATH, "w", encoding="utf-8") as f:
        json.dump(movie_genres, f, ensure_ascii=False, indent=2)
    log.info(f"movie_genres.json salvo com {len(movie_genres)} relacoes")
